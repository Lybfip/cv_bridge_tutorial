...
std::string dt_node_name = "detect_target";    // デフォルトノード名
int counter = 0;
...
public:
    ...
    {
        nh_.getParam(dt_node_name + "/offset", camera_offset);     // offsetパラメータの取得
        std::cout << "[" + dt_node_name + "] カメラオフセット=" << camera_offset << std::endl;
        // 入力ビデオフィードのサブスクライブと出力ビデオフィードのパブリッシュ
        image_sub_ = it_.subscribe("/cv_camera/image_raw", 1, &ImageConverter::imageCb, this);
        image_pub_ = it_.advertise(dt_node_name + "/monitor", 1);
        angle_pub_ = nh_.advertise<std_msgs::Float32>(dt_node_name + "/angle", 1);
        dist_pub_ = nh_.advertise<std_msgs::Float32>(dt_node_name + "/distance", 1);
        valid_pub_ = nh_.advertise<std_msgs::Bool>(dt_node_name + "/valid", 1);
    }
    ~ImageConverter()
    {
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        {
            std::cout << "[" + dt_node_name + "] 画像を取得できませんでした";
            return;
        }
        ...
        for (int p1 = 0; p1 < cc - 1; p1++) {
            for (int p2 = p1 + 1; p2 < cc; p2++) {
                ...
                if (targets[p1].cx == targets[p2].cx) {
                    std::cout << "[" + dt_node_name + "] x座標が同一です. p1=" << targets[p1].cx << ", p2=" << targets[p2].cx << std::endl;
        ...
        if (element1 == -1 || element2 == -1){
            ...
            std::cout << "[" + dt_node_name + "] ターゲットが見つかりません。" << std::endl;
        }
        else{
        ...
        // std::cout << ("処理時間= %u.09%u", ros_duration.sec, ros_duration.nsec) << std::endl;
    }
...
{
    ros::init(argc, argv, dt_node_name);
    // ノード名の取得
    dt_node_name = ros::this_node::getName();

    ImageConverter ic;

    std::cout << "[" + dt_node_name + "] ノードを起動しました。" << std::endl;
